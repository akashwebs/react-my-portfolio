        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 px-8 text-start my-8'>
            ...
            <div id='signle-port' className='p-8 mt-48 md:mt-0'>
                ...
                <div>
                    ...
                    {
                        proejct?.language?.map(l => <button className='px-8 py-3 bg-red-900 mr-2 mb-2 rounded-md'>{l}</button>)
                    }
                </div>
                <div>
                    <p className="text-3xl mt-8 mb-5">Link: </p>
                    <a className='btn' href={`${proejct?.live[0]}`} target="_blank" rel="noopener noreferrer" >Live</a>
                    <a className='btn' href={`${proejct?.live[1]}`} target="_blank" rel="noopener noreferrer" >Github(Client)</a>
                    {
                        proejct?.live[2] && <a className='btn mr-5 mb-5 ' href={`${proejct?.live[2]}`} target="_blank" rel="noopener noreferrer" >
